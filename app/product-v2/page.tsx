...
const INVERTEBRÉ_DEMO: InvertebreProduct = {
  ...
  imageUrl: "/boite.png",
  category: "Isopodes",
  description: "Espèce robuste et facile d'entretien, idéale pour débuter dans l'élevage d'isopodes. Ces détritivores sont parfaits pour maintenir l'humidité dans les terrariums et participer activement à la décomposition de la matière organique.",
  stock: 25,
...
const ACCESSOIRE_DEMO: AccessoireProduct = {
  ...
  imageUrl: "/boite.png",
  category: "Transport",
  description: "Boîte de transport isotherme spécialement conçue pour le transport sécurisé de vos invertébrés. Isolation thermique optimale et aération contrôlée pour des trajets sans stress.",
  stock: 50,
  features: [
...
function Section({ 
  ...
  return (
    ...
    >
      ...
      >
        <h2 className={`${compact ? "text-xs" : "text-xl"} font-bold text-foreground uppercase tracking-tight flex items-center gap-2 flex-1`}>
          <span className={`w-1 ${compact ? "h-3" : "h-4"} bg-primary`} />
          {title}
        </h2>
        {collapsible && (
          <span className={`text-primary transition-transform text-sm ${isOpen ? "rotate-180" : ""}`}>▼</span>
        )}
      </div>
      {isOpen && (
        <div className={collapsible ? `${compact ? "mt-1.5" : "mt-3"}` : ""}>
          {children}
        </div>
      )}
    </div>
...
function InvertebreDetails({ product }: { product: InvertebreProduct }) {
  return (
    <Section title="💡 Conseils d'élevage" collapsible>
      <ul className="space-y-1">
        {product.tips.map((tip, index) => (
          <li key={index} className="flex gap-2 text-xs text-muted-foreground p-1.5 bg-primary/5 border border-primary/15 rounded-sm leading-relaxed">
...
function AccessoireDetails({ product }: { product: AccessoireProduct }) {
  ...
  return (
    <div className="space-y-1">
      {/* Caractéristiques */}
      <Section title="Caractéristiques" variant="premium" compact collapsible>
        <ul className="space-y-0.5">
          {product.features.map((feature, index) => (
            <li key={index} className="flex items-start gap-2 text-muted-foreground text-xs\">
          ...
          <div className="space-y-2">
            {product.options.map((option) => (
              <div key={option.name}>
                ...
                <select 
                  className="w-full px-4 py-3 bg-card border border-primary/40 rounded text-foreground focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 md:gap-4 items-start">
          ...
          <div className="flex flex-col gap-2 lg:col-span-1 order-1 lg:order-1">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="flex items-center gap-1">
                  <Button
                    variant="outline"
                    size="icon"
                    className="h-8! w-8! border-primary/50 hover:bg-primary/20"
                ...
                <span className="text-xs text-muted-foreground ml-auto">{product.stock} dpos.</span>
              </div>
              ...
              <div className="pt-1 border-t border-primary/30 flex items-center justify-between text-xs mt-2">
                <span className="text-muted-foreground uppercase tracking-wider">Total</span>
                <span className="font-black text-primary font-mono">{(product.price * quantity).toFixed(2)}€</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5 lg:col-span-2 order-2 lg:order-2">
            ...
            <div className="px-2 py-1.5">
              <h1 className="text-sm md:text-base font-black text-foreground uppercase tracking-widest leading-tight">
                {product.name}
              </h1>
              <div className="h-px bg-linear-to-r from-primary/60 to-transparent mt-1" />
            </div>

            {/* Description */}
            <Section title="À propos"  collapsible={true}>
