...
export function AdminKpiCard({ label, value, hint }: AdminKpiCardProps) {
  return (
    <article className="relative flex min-h-42.5 flex-col justify-center overflow-hidden border border-admin-border bg-admin-panel p-4.75">
      <span
      ...
      {hint && <p className="admin-type-micro mt-3 text-admin-muted">{hint}</p>}
    </article>
  );
}
